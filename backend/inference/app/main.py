"""FastAPI application for the inference service. Structure and health checks only."""
...
def create_app() -> FastAPI:
    configure_logging()
    app = FastAPI(title="KrishiTech inference", version="0.1.0")
    ...
    async def readyz() -> JSONResponse:
        """Readiness. Settings load from the environment."""
        try:
            settings = get_settings()
        except ValidationError as exc:
            return JSONResponse(
